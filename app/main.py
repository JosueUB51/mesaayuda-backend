...
def descargar_formato_vpn():
    ...
    if not os.path.exists(path):
        return JSONResponse(
            status_code=404,
            content={"error": "vpn_formato.pdf no encontrado en app/static"}
        )
...
def descargar_formato_correo():
    ...
    if not os.path.exists(path):
        return JSONResponse(
            status_code=404,
            content={"error": "correo_formato.pdf no encontrado en app/static"}
        )
...
# VALIDADOR DE PDF
# --------------------------------
def validar_pdf(file: UploadFile, tipo: str):
    ...
    if tipo == "vpn" and "vpn" not in nombre:
        return JSONResponse(
            status_code=400,
            content={"error": "El archivo no corresponde al formato VPN"}
        )

    if tipo == "correo" and "correo" not in nombre:
        return JSONResponse(
            status_code=400,
            content={"error": "El archivo no corresponde al formato de correo"}
        )
...
async def upload_vpn_form(
    ...
    return {
        "ok": True,
        "message": "Formato VPN subido correctamente",
        "file": safe_name
    }
...
async def upload_correo_form(
    ...
    return {
        "ok": True,
        "message": "Formato Correo subido correctamente",
        "file": safe_name
    }
...
async def root():
    return {
        "status": "Mesa de Ayuda IA funcionando correctamente"
    }